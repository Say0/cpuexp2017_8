...
let lexbuf outchan outmiddlechan_first outmiddlechan_second outmiddlechan_third l = (* バッファをコンパイルしてチャンネルへ出力する (caml2html: main_lexbuf) *)
  Id.counter := 0;
...
let file f = (* ファイルをコンパイルしてファイルに出力する (caml2html: main_file) *)
  ...
  try
    lexbuf outchan outmiddlechan_first outmiddlechan_second outmiddlechan_third (Lexing.from_channel inchan);
    close_in inchan;
    close_out outchan;
    close_out outmiddlechan_first;
    close_out outmiddlechan_second;
    close_out outmiddlechan_third
  with e -> (close_in inchan; close_out outchan; close_out outmiddlechan_first; close_out outmiddlechan_second; close_out outmiddlechan_third; raise e)
